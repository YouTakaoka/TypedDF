...
def main():
    ...
    tdf: TypedDF[GenericIndexType[int], Human] = TypedDF.from_df(df, Human, GIT)  # OK
    print(tdf)

    df2: pd.DataFrame = pd.read_csv('example/human_list2.csv')
    tdf2: TypedDF[GenericIndexType[int], Human2] = TypedDF.from_df(df2, Human2, GIT) # OK
    print(tdf2)
    ...
    try:
        tdf3: TypedDF[GenericIndexType[int], Human2] = TypedDF.from_df(df, Human2, GIT) #Runtime error!
        print(tdf3)
    except TypeError as e:
        print(e)
    
    try:
        tdf4: TypedDF[GenericIndexType[int], Human] = TypedDF.from_df(df2, Human, GIT) #Runtime error!
        print(tdf4)
    except TypeError as e:
        print(e)
